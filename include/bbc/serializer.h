...
#include "utils/traits/serializable_container.h"

#include "interfaces/commands/description/enum_maker.h"
...
std::vector<uint8_t> Serialize(const T& t)
{
    ...
    else if constexpr (std::derived_from<T, Frasy::Internal::Enum>)
    {
        return Serialize(static_cast<typename T::underlying_type>(t));
    }
    else
    {
        std::vector<uint8_t> out;
